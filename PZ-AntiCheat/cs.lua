...
AddEventHandler('onResourceStart', function(resourceName)
  ...
  print('PZ AntiCheat > Zapnutý')
end)
AC = {
  'tac_garbagejob:pay',
  'tac_truckerjob:pay',
  'tac_gopostaljob:pay',
  'tac_slotmachine:sv:2',
  'tac_carthief:pay',
  'tac_ranger:pay',
  'tac_pizza:pay',
  'LegacyFuel:PayFuel',
  'xaries gang 3*',
  'tac-qalle-jail:jailPlayer',
  'xseira.CreateSubMenu("SelfMenu", "MainMenu", "Self Menu3")',
  'tac-qalle-hunting:sell',
  'tac-qalle-hunting:reward',
  'tac_jebackoguszkaa_jailer:sendToJail',
  'tac_jail:sendToJail',
  'tac_jailer:sendToJail',
}
...
AddEventHandler('yung_ac:ban', function(argument, powod, typkary)
  ...
  DropPlayer(source, '⚡️ Project PZ')
...
AddEventHandler('playerConnecting', function (playerName,setKickReason)
  ...
  for i = 1, #ListaBanow, 1 do
    ...
    and (tostring(ListaBanow[i].typkary)) == 'BAN' then
      ...
      setKickReason('✈ Byl jsi deportovan na bananové ostrový. [AntiCheat Project-PZ]')
      print('[PZ AntiCheat]: ' .. GetPlayerName(source) .. ' - sa chce pripojit aj keď bol deportovaný na bananové ostrový')
      CancelEvent()
    end
  end
end)
...
function AClogkick(source,arg,kick)
  ...
  for k,v in ipairs(GetPlayerIdentifiers(source))do
    ...
    local n= {
        ...
        ["footer"]=
        {
            ["text"]="Terveisin Nicke"},
            ["timestamp"] = os.date('!%Y-%m-%dT%H:%M:%S'),
        }
    }

    PerformHttpRequest(Config.webhook,function(f,o,h)end,'POST',json.encode({username="Nyt on epäilyttävää toimintaa!!!",embeds=n}),{['Content-Type']='application/json'})

  if kick then
  zapiszsql(source, arg, 'BAN')
  local j=GetPlayerName(source)
  local m=GetPlayerIdentifier(source)
	DropPlayer(source, '⚡️ PZ AntiCheat Detektoval zakazanú vec ⚡️')
  end
end
...
AddEventHandler('chatMessage', function(source, n, message)
  for k,n in pairs(lol) do
    if string.match(message:lower(),n:lower()) then
      AClogkick(source,"Kirjoitti: "..n,true)
    end
  end
end)
